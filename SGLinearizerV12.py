...
def createValuesImage(dutGenName, stdGenName, asset, reCharacterizationInterval, resX, resY, startX, startY,
                      lineSpacing):
    ...
    y += lineSpacing
    d.text((x, y), "----------------------------------------------------------".format(asset), font=fnt,
           fill=(255, 255, 0))
    y += lineSpacing
...
if os.path.isfile(measLog):
    if useTKINTER == True:
        guiTools.popupMsg("Measurement log file \"" + str(
            measLog) + "\" already exists; backup the original, if you wish to save it, before proceeding.")
    else:
        userPrompt("Measurement log file \"" + str(
            measLog) + "\" already exists; backup the original, if you wish to save it, before proceeding. Press Enter to Continue...")
    os.remove(measLog)
...
if os.path.isfile(dutCorFile):
    if useTKINTER == True:
        guiTools.popupMsg("DUT Correction Data file \"" + str(
            dutCorFile) + "\" already exists; backup the original, if you wish to save it, before proceeding.")
    else:
        userPrompt("DUT Correction Data file \"" + str(
            dutCorFile) + "\" already exists; backup the original, if you wish to save it, before proceeding. Press Enter to Continue...")
    os.remove(dutCorFile)
    writeLog("DUT Correction Data file > {} < already existed - deleted old correction file".format(str(dutCorFile)),
             logFile)
else:
...
writeLog("charFreq: " + str(charFreq), logFile)
...
writeLog("sensorMaxPow: " + str(sensorMaxPow), logFile)
...
writeLog("reCharacterizationInterval: " + str(reCharacterizationInterval), logFile)
uom = readConfigFile(configFile, "uom")
writeLog("uom: " + str(uom), logFile)
...
writeLog("samplingQuantity: " + str(samplingQuantity), logFile)

refPow = readConfigFile(configFile, "referencePower")
writeLog("refPow: " + str(refPow), logFile)

dutTargetAcc = readConfigFile(configFile, "dutTargetAcc")
dutTargetAcc = float(dutTargetAcc)
writeLog("dutTargetAcc: " + str(dutTargetAcc), logFile)

resX = readConfigFile(configFile, "resX")
resX = int(resX)
writeLog("resX: " + str(resX), logFile)

resY = readConfigFile(configFile, "resY")
resY = int(resY)
writeLog("resY: " + str(resY), logFile)

startX = readConfigFile(configFile, "startX")
startX = int(startX)
writeLog("startX: " + str(startX), logFile)

startY = readConfigFile(configFile, "startY")
startY = int(startY)
writeLog("startY: " + str(startY), logFile)

lineSpacing = readConfigFile(configFile, "lineSpacing")
lineSpacing = int(lineSpacing)
writeLog("lineSpacing: " + str(lineSpacing), logFile)
...
writeLog("steps: " + str(steps), logFile)
...
writeVisa(dutGenVisa, dutGenOff)
time.sleep(1)
...
writeLog("Performing Sensor Zero and Cal", logFile)
print("Connect the power sensor to the {} output".format(stdGenName))
userPrompt("Press enter to continue...")
print("Zero Power Meter/Sensor...")
writeVisa(pMeterVisa, pmZero)
time.sleep(0.25)
response = queryVisa(pMeterVisa, pmOpc)
print("Meter Zero Response: ", response)
time.sleep(1)
charFreq = 50000000
powerLevel = 0
writeVisa(stdGenVisa, stdGenFreq.replace("<val>", str(charFreq)))
writeVisa(stdGenVisa, stdGenPow.replace("<val>", str(powerLevel)))
writeVisa(stdGenVisa, stdGenOn)
print("Cal Power Meter/Sensor...")
writeVisa(pMeterVisa, pmCal)
time.sleep(0.25)
response = queryVisa(pMeterVisa, pmOpc)
print("Meter Cal Response: ", response)
time.sleep(1)

# End PM Config Routine --------------------------------------

writeVisa(stdGenVisa, stdGenOn)
writeLog("Performing baseline characterization", logFile)
clear()
userInterfaceHeader(programName, version, cwd, logFile, "Performing Baseline Characterization")
baseLineSampleQty = 100
spinner = 0
msmts = []
for j in range(baseLineSampleQty):
    if spinner == 4:
        spinner = 0
    if spinner == 0:
        progress = "|"
...
writeVisa(stdGenVisa, stdGenOff)
baselineCharValue = statistics.mean(msmts)
writeLog("Completed baseline characterization", logFile)
...
# Zeroing Routine -------------------------------------------
print("Connect the power sensor to the output of the {} and the {} via a power splitter:".format(stdGenName,dutGenName))
print("- Connect the {} and the {} to the splitter output ports.".format(stdGenName,dutGenName))
print("- Connect the power sensor to the splitter input port")
userPrompt("Press enter to continue...")
# writeVisa(stdGenVisa,stdGenOff)
# response = queryVisa(pMeterVisa, pmRead, "float")
...
print("Characterizing splitter...")
charFreq = 50000000
powerLevel = 0
writeVisa(stdGenVisa, stdGenFreq.replace("<val>", str(charFreq)))
writeVisa(stdGenVisa, stdGenPow.replace("<val>", str(powerLevel)))
writeVisa(stdGenVisa, stdGenOn)
writeLog("Performing Splitter Characterization", logFile)
baseLineSampleQty = 100
spinner = 0
msmts = []
for j in range(baseLineSampleQty):
...
writeVisa(stdGenVisa, stdGenOff)
splitterInsertionLoss = (statistics.mean(msmts) - baselineCharValue)
writeLog("Completed Splitter Characterization", logFile)
writeLog("splitterInsertionLoss {}".format(splitterInsertionLoss), logFile)
...
for index, i in enumerate(steps):
    ...
    # Stabilize the STD generator and sensor...
    writeVisa(stdGenVisa, stdGenFreq.replace("<val>", str(charFreq)))
    writeVisa(stdGenVisa, stdGenOn)
    writeVisa(stdGenVisa, stdGenPow.replace("<val>", str(i)))
    clear()
    userInterfaceHeader(programName, version, cwd, logFile, "Standard Generator Transfer Characterization In Process")
    spinTimerFunction(settlingTime, "Stabilizing {} Output Level at {} dBm...".format(stdGenName,i))
    ...
    for j in range(samplingQuantity):
        ...
        spinner += 1
    writeVisa(stdGenVisa, stdGenOff)
    # print("msmts {}".format(msmts))
    tempFloat = statistics.mean(msmts)
    # print("Std tempFloat: {}".format(tempFloat))
    msd = tempFloat - splitterInsertionLoss
    print("Std Generator Measured {} at {} step".format(msd,i))
    writeLog("stdGen msd {} at {} step".format(msd,i), logFile)
    ...
    toLog = "{}- ,{}, dBm Step,{}, dBm- Measured ,{}, dBm (,{}, Watt Step ,{}, Watt)".format(stdGenName, logPow, logPow,
                                                                                             msd, linPow, linMsd)
    writeLog(toLog, measLog)

    # Stabilize the DUT generator and sensor...
    ...
    clear()
    userInterfaceHeader(programName, version, cwd, logFile, "DUT Generator Transfer Characterization In Process")
    spinTimerFunction(settlingTime, "Stabilizing {} Output Level at {} dBm...".format(dutGenName,i))
    ...
    for j in range(samplingQuantity):
        ...
        msmts.append(response)
        spinner += 1
    msd = statistics.mean(msmts) - splitterInsertionLoss
    print("Dut Initial Measurement: {}".format(msd))
    ...
    for j in range(5):
        ...
        print("stdLogVals: {}".format(stdLogVals[index]))
        ...
        if abs(tempDiff) > dutTargetAcc:
            ...
            for j in range(samplingQuantity):
                ...
                spinner += 1
            msd = statistics.mean(msmts) - splitterInsertionLoss
            print("Adjustment Loop: {}".format(msd))
    print("dutGen msd {} at {} step".format(msd, i))
    writeLog("dutGen msd {} at {} step".format(msd, i), logFile)
...
writeLog("Completed linearity characterization Process", logFile)

writeLog("Creating image file...", logFile)
fileName = createValuesImage(dutGenName, stdGenName, asset, reCharacterizationInterval, resX, resY, startX, startY,
                             lineSpacing)
